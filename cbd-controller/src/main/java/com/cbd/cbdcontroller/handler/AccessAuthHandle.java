...
//        authentication(userEntity, accessAuthEntity);
    ...
    private void checkLogin(UserEntity userEntity, AccessAuthEntity accessAuthEntity) {
        ...
        if (accessAuthEntity.isLogin() && userEntity==null) {
            throw new GlobalException(CodeMsg.UNLOGIN);
        }
    }
    ...
    private AccessAuthEntity getAccessAuthEntity(String method, String url) {
        // 获取所有接口的访问权限
       Map<String,AccessAuthEntity> accessAuthMap = redisService.hmget(AccessAuthKey.Access_Auth,"all",AccessAuthEntity.class);
        ...
        // 没有该接口
        throw new GlobalException(CodeMsg.ERROR_404);
    }
    ...
    private UserEntity getUserEntity(String sessionID) {
        ...
        // 获取UserEntity
        UserEntity userEntity = redisService.get(AccessAuthKey.SessionID,sessionID,UserEntity.class);
        if (userEntity==null) {
            return null;
        }
        return userEntity;
